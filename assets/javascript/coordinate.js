...
const takeUserInput = function () {
    var city_longitude = search_input.value;
        if (city_longitude.trim() === "") {
        alert("Error: City name cannot be empty!")
    }
    
    var city_latitude = search_input2.value;
        if (city_latitude.trim() === "") {
        alert("Error: City name cannot be empty!")
    }
    
    getCityCurrentWeather(city_longitude, city_latitude);
}

//We pass this input(specified city) to the api to get the response/details about the weather back
const getCityCurrentWeather = function (city_longitude, city_latitude) {

    //the api_url looks for the specified city from the input to get its current weather info
    const api_url = 'https://api.openweathermap.org/data/2.5/weather?lat=' + city_latitude + "&lon=" + city_longitude + '&appid=f146799a557e8ab658304c1b30cc3cfd&units=metric';
...
const displayCurrentWeatherResults = function(responseFromApi) {
    city.innerHTML = "Location: " + responseFromApi.coord.lon + "° and " + responseFromApi.coord.lat + "°";
